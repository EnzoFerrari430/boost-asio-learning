...
void CSession::Start()
{
    ...
    _socket.async_read_some(boost::asio::buffer(_data, MAX_LENGTH),
        boost::asio::bind_executor(_strand, std::bind(&CSession::HandleRead, this, std::placeholders::_1, std::placeholders::_2, SharedSelf())));
...
{
    //std::cout << "The CSession(uuid: " << _uuid << " ) HandleRead thread is : " << std::this_thread::get_id() << std::endl;
    if (!error)
    {
        ...
        while (bytes_transferred > 0)
        {
            if (!_b_head_parse)
            {
                ...
                if (bytes_transferred + _recv_head_node->_cur_len < HEAD_TOTAL_LEN)
                {
                    ...
                    ::memset(_data, 0, MAX_LENGTH);
                    _socket.async_read_some(boost::asio::buffer(_data, MAX_LENGTH),
                        boost::asio::bind_executor(_strand, std::bind(&CSession::HandleRead, this, std::placeholders::_1, std::placeholders::_2, _self_shared)));
                    return;
                }
